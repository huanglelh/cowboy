...
conv_file(FileIn, FileOut) ->
    case file:open(FileIn, [read]) of
        {ok, Device} ->
            ...
            Output = conv(Input),
            io:format("erlmarkdown conv_file is:~p~n",[Output]),
            write(FileOut, Output);
        _ ->
            error
    end.

conv_file_with_style(FileIn, FileOut,FileStyle) ->
    io:format("conv_file_with_style FileIn is ~p~n",[FileIn]),
    io:format("conv_file_with_style FileOut is ~p~n",[FileOut]),
    io:format("conv_file_with_style FileStyle is ~p~n",[FileStyle]),
    case file:open(FileIn, [read]) of
        {ok, Device} ->
            Input = get_all_lines(Device,[]),
            Output = conv(Input),
            case is_list(Output) of
                true ->
                    OutputBinary = binary:list_to_bin(Output),
                    generate_html(FileOut,FileStyle,OutputBinary);
                false->
                    generate_html(FileOut,FileStyle,Output)
            end;
        _ ->
            error
    end.
...
generate_html(FilePath,StyleFilePath,BodyBinary)->
    case file:read_file(StyleFilePath) of
        {ok, _Binary} ->
            io:format("erlmarkdown conv_file style is:~p~n",[_Binary]),
            CompleteBinary = binary:replace(_Binary,<<"<body>">>,BodyBinary,[{insert_replaced,0}]),
            write(FilePath, CompleteBinary);
        _Other->
            write(FilePath, BodyBinary)
    end.
%%============================================================================
%% Internal Functions
%%============================================================================
